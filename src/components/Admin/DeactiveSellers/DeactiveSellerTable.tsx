...
import {   SellerTableProps } from "../../../types/types";
...
const DeactiveSellerTable = ({sellers}:SellerTableProps) => {
  
  const [updateUserStatus, { isLoading: isVendorStatusLoading }] =
  ...
  const handleActivateSeller = async(id:string) => {
          const result = await Swal.fire({
            title: 'Activate this seller?',
            text: "This seller will be marked as active and allowed to access the platform again.",
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#21324A',
            cancelButtonColor: '#F44336',
            confirmButtonText: 'Yes, activate seller',
            cancelButtonText: 'Cancel',
          });
          if(result.isConfirmed){  
            setActivatingUserId(id)
            await updateUserStatus({ id: id, data: { status: "active" } }).unwrap();
              ...
              <tr key={seller._id}>
                ...
                <td className={classes}>
                  <div className="flex items-center gap-4">
                    <Tooltip content="Activate Seller">
                      <IconButton
                        variant="filled"
                        aria-label="Edit Status"
                        onClick={() => handleActivateSeller(seller._id)}
                        {...(undefined as any)}
                        disabled={
                          (isDeleteLoading && deletingUserId === seller._id)
                           ||
                          (isVendorStatusLoading && activatingUserId === seller._id)
                        }
                       >
